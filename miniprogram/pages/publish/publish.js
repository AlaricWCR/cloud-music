...
Page({
  ...
  onInput(event) {
    ...
    if (wordsNum >= event.detail.value.length) {
  ...
  send() {
    ...
    // openid小程序端操作云数据库可以自动获得，时间可以获取数据库服务器时间

    // 1、对文字输入内容做判空操作，（没有输入或者输入的都是空格，所以先trim一下）
    if (content.trim() === '') {
      wx.showModal({
        title: '请输入内容',
        content: '',
   ...
   // 存入到云数据库
   // Promise.all的resolve回调执行实在所有输入的promise的resolve回调都结束
